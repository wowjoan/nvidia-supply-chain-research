...
本專案不是「產生一段摘要」，而是把每一條關係結論、支持證據、時效性、方向與可解釋的
信賴度評分連接起來，讓 reviewer 可以理解、運行、並追溯每一個判斷。

> 這是一份求職挑戰交付物（ARTi Product Manager 職缺），由 Joan Chou 完成。
> 免責聲明：本文件與資料集僅為研究與工程能力展示，基於公開資訊整理，**不構成任何投資建議**。
